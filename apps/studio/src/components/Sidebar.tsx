...
import AsyncAIImage from '../../public/asyncapi-ai-mini-logo.svg';
import Image from 'next/image';

function updateState(
  panelName: keyof PanelsState['show'],
  type?: PanelsState['secondaryPanelType']
) {
...
export const Sidebar: FunctionComponent<SidebarProps> = () => {
  ...
  let navigation: NavItem[] = [
    // navigation
    {
      ...
      onClick: () => {
        if (!show.primarySidebar) {
          updateState('primarySidebar'); // Enable primarySidebar
          if (show.aiPanel) {
            updateState('aiPanel'); // Disable aiPanel if it's active
          }
        } else {
          updateState('primarySidebar'); // Toggle off primarySidebar
        }
      },
  ...
  return (
    <div
      className='flex flex-col bg-gray-800 shadow-lg border-r border-gray-700 justify-between'
      id='sidebar'
    >
      <div className='flex flex-col'>
          ...
          >
            ...
            >
              <div
                className={
                  item.isActive
                    ? 'bg-gray-600 p-2 rounded text-white'
        ...
        >
          <button
            onClick={() => {
              if (!show.aiPanel) {
                updateState('aiPanel'); // Enable aiPanel
                if (show.primarySidebar) {
                  updateState('primarySidebar'); // Disable primarySidebar if it's active
                }
              } else {
                updateState('aiPanel'); // Toggle off aiPanel
              }
            }}
            className={'flex text-sm focus:outline-none border-box p-2'}
            type='button'
            data-test='button-ai-sidebar'
          >
            <div
              className={
                show.aiPanel
                  ? 'bg-gray-600 rounded text-white'
